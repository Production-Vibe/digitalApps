...
- **`main`** — новый стек: Node/Express (TypeScript) + EJS + Prisma + PostgreSQL,
  ...
  **Два контура БД (важно):** стендовый/комpose-контур — Docker-PG на
  `localhost:5432` host-порт (в контейнере — БД `digital_narad`, юзер `user`);
  дев-контур (`npm run dev`, `server/.env`) — **локальный Windows-PostgreSQL** на
  `localhost:5432`/БД `MOSD`, `production_user` (на момент 14.09.2026 локальный PG
...
3. (опц.) Разрулить конфликт порта 5432: локальный Windows-PG (дев-контур `MOSD`)
   держит порт, Docker-db (стенд `digital_narad`) тоже маппит 5432 — перевести один
   из контуров на другой порт и зафиксировать в STATUS.
