...
- [ ] **E2E покрывает вход+рендер+refresh+вкладки** (26/26 PASS) и **полный
  бизнес-цикл** (`test_bizcycle.py`, 37/37 PASS ×5). В харнесс пока не вынесены
  кейсы «закрытая смена в истории → форма открытия видна», «уведомления»
  (бейдж/тост/dropdown) и содержимое вкладок/графиков аналитики мастера
  (покрыты probe-прогонами, 14/14 и ALL PASS).
...
1. **Дополнить E2E-харнесс** кейсами «закрытая смена в истории → форма открытия
   видна», «уведомления» (бейдж/тост/dropdown) и содержимое вкладок/графиков
   аналитики мастера (сейчас — ручные probe-прогоны).
