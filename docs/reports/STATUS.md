...
- [ ] **E2E покрывает только вход+рендер+refresh** (18/18 PASS); полный
  ...
  кандидат следующего этапа после аналитики мастера. Кейс «закрытая смена в
  истории → форма открытия видна» покрыт probe-прогоном (14/14), в харнесс не
  вынесен.
- [ ] **Master урезан против GAS:** согласованный состав «полной картины» —
  оперативная сводка + дерево номенклатуры + отчёты/история (очередь печати из
  объёма выведена; `PrintQueue` продолжает писаться при выдаче). Реализация —
  Этап 3.
...
1. **Этап 3: аналитика мастера.** Согласованный состав — оперативная сводка +
   дерево номенклатуры + отчёты/история. Архитектура: единый серверный источник
   метрик (`GET /api/analytics/dashboard`, одна агрегация из Launches/WorkOrders/
   Shifts/Transitions/ClosedOrders/Catalog/Equipment), виджеты = презентация без
   дублирования; «загруженность ПА» (активные запуски) и «занятость станков»
   (открытые смены) — разные метрики с чётким названием. Идёт через spec →
   реализацию (этот этап проектируем вместе).
