...
- Числа: единое «умное» округление `roundSmart(n)` (`server/src/lib/round.ts` +
  JS-зеркало в `server/src/public/ui.js`) — `|x| ≥ 0.01` → максимум 2 знака,
  `0 < |x| < 0.01` → до 1 значащей цифры, `0 → 0`, хвостовые нули в отображении
  убираются. Применяется на записи в БД, в агрегатах аналитики и во всех
  интерполяциях UI (`fmtNum`/`roundSmart`).
- ID: `Н-yyMMdd-HHmmss` (наряд), `ЗП-…` (запуск), `СМ-…` (смена); переходы —
  `005, 010, 015…` (`server/src/lib/id.ts`). Метка времени — секундная, поэтому
  при создании нескольких сущностей в одну секунду к ID добавляется суффикс
  `-N` (`Н-260917-072540`, `Н-260917-072540-2`, …) — уникальность PK сохраняется,
  обычный формат не меняется.
