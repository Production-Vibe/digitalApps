...
`WorkOrders`. Листы `Наряды`/`Переходы`/`Закрытые` (раздел VBA-интерфейс ниже) —
учётные проекции для VBA/отображения/печати и пишутся из канона, а не
«впервые». Реализация отдельным планом.
...
## Учётная проекция (листы `Наряды`, `Переходы`, `Закрытые`) — VBA-интерфейс

Проекция канона `WorkOrders` (ADR-003): предназначены для VBA/отображения/печати.
...
| `WorkOrders.Статус` | `created` → `in_progress` → `waiting_otk` → `closed` (канон наряда, ADR-003) |
