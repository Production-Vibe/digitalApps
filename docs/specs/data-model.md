...
### `Наряды`
6 колонок (`naryadRowToObject`, `modules/NaryadAPI.md:45-54`): `Номер наряда`,
`Деталь`, `Код детали`, `Кол-во`, `Статус`, `Дата`.
Статусы: `created`, `in_progress`, `waiting_otk`, `closed`.

### `Переходы`
12 колонок (`transitionRowToObject`, `modules/NaryadAPI.md:56-71`): `Наряд`,
`№ п/п` (005, 010…), `Описание`, `Оператор`, `Время`, `Плавка`, `Станок`,
`Кол-во`, `Статус`, `Дата`, `Принято`, `Брак`.
Статусы: `in_progress`, `completed`, `checked`.
`createTransition` автонумерует: следующий `№` = max+5 (или 005).

### `Закрытые`
7 колонок (`getClosingInfo`, `modules/NaryadAPI.md:84-101`): `Наряд`,
`Принято всего`, `Брак всего`, `Причина брака`, `Комментарий`, `Кем закрыт`,
`Дата`.
...
| `Наряды.Статус` | `created` → `in_progress` → `waiting_otk` → `closed` |
