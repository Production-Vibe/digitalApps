...
## Источник фактов наряда (ADR-003, вариант A)

- **Канон** — `WorkOrders` (12 колонок, `data-model.md`). Все факты наряда
  записываются здесь «впервые».
- **Учётная проекция** — `Наряды`/`Переходы`/`Закрытые` (для VBA/отображения/
  печати) — производны от канона; бизнес-логика в них напрямую не пишет.
- Кто пишет в какой лист — по `data-model.md`; при реализации write-path
  (операторский поток, ОТК, VBA `create_naryad` → legacy) — отдельный план.
